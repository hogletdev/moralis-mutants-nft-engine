...
const writeMetaData = (metadataList) => {
    if (process.env.JSON_INCIDENT == true) {
        fs.writeFileSync(
...
const writeMetaDataMap = (metadataList) => {
    if (process.env.JSON_INCIDENT == true) {
        fs.writeFileSync(
            './output/_metadataMap.json',
            JSON.stringify(metadataList, null, 2)
        )
    } else {
        fs.writeFileSync(
...
const saveMetadataLocally = async (
    editionCount,
    editionSize,
    imageDataArray
) => {
    ...
    for (let i = 1; i < editionCount; i++) {
        ...
        metadataList.push(nftMetadata)
        metadataMap[nftMetadata.name] = nftMetadata
        fs.writeFileSync(
            `./output/${filename}`,
            JSON.stringify(metadataList.find((meta) => meta.edition == i))
        )
